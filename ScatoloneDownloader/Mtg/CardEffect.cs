namespace ScatoloneDownloader.Mtg
{
    ...
    public enum CardEffect
    {
        ...
        CardAdvantage   = 1 << 9,   // a card the opponent does not get: draw two or more, a draw you can go back to (an activated ability or a recurring trigger — an ETB fires once and is a cantrip), or "draw a card for each X", which is the same thing counted the other way round. PARITY is not advantage, and it wears three disguises: a loot draws and discards in one breath (that is Filter alone, ruled 2026-09-04), cycling pays a card to replace itself, and an ability that sacrifices its own permanent runs once. A CLUE is the borderline case and splits by economy: it has to be cashed for {2}, so a lone one is a rider and only a repeatable source of them earns the tag, while an IMPULSE ("exile the top card, you may play it") is a free extra card the moment it resolves and counts even once. Ruled 2026-09-11, parity carved out and the Clue/impulse line drawn 2026-09-15.
        Filter          = 1 << 10,
        Reanimate       = 1 << 11,
